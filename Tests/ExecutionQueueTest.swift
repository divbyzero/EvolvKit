...
class ExecutionQueueTest: XCTestCase {
    ...
    func testExecuteAllWithValuesFromAllocations() {
        let key = "pages.testing_page.header"
        let defaultValue = "red"
        let exMock1 = ExecutionMock(key: key, defaultValue: __N(defaultValue), participant: participant, store: mockAllocationStore, closure: printSomething)
        let exMock2 = ExecutionMock(key: "pages.testing_page.header", defaultValue: __N("red"), participant: participant, store: mockAllocationStore, closure: doSomething)
    ...
    func testExecuteAllWithValuesFromDefaults() {
        ...
        let defaultValue = "red"
        let exMock1 = ExecutionMock(key: key, defaultValue: __N(defaultValue), participant: participant, store: mockAllocationStore, closure: printSomething)
        let exMock2 = ExecutionMock(key: key, defaultValue: __N(defaultValue), participant: participant, store: mockAllocationStore, closure: doSomething)
